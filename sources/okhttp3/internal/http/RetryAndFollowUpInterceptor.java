...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    private Request followUpRequest(Response response, Route route) throws IOException {
        ...
        if (iCode == 307 || iCode == 308) {
            if (!strMethod.equals(g.L) && !strMethod.equals("HEAD")) {
                return null;
            }
        } else {
            if (iCode == 401) {
            ...
            if (iCode == 503) {
                if ((response.priorResponse() == null || response.priorResponse().code() != 503) && retryAfter(response, IntCompanionObject.MAX_VALUE) == 0) {
        ...
        if (!this.client.followRedirects() || (strHeader = response.header(g.F)) == null || (httpUrlResolve = response.request().url().resolve(strHeader)) == null) {
            return null;
        }
    ...
    private int retryAfter(Response response, int i) {
        ...
        return strHeader == null ? i : strHeader.matches("\\d+") ? Integer.valueOf(strHeader).intValue() : IntCompanionObject.MAX_VALUE;
    }

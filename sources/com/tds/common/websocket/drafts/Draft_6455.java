...
public class Draft_6455 extends Draft {
    ...
    public HandshakeState acceptHandshakeAsServer(ClientHandshake clientHandshake) throws InvalidHandshakeException {
        if (readVersion(clientHandshake) != 13) {
            LogUtil.logD("acceptHandshakeAsServer - Wrong websocket version.");
        ...
        while (true) {
            ...
            if (next.acceptProvidedExtensionAsServer(fieldValue)) {
                ...
                LogUtil.logD("acceptHandshakeAsServer - Matching extension found: {}", this.extension);
                break;
            }
        }
        ...
        LogUtil.logD("acceptHandshakeAsServer - No matching extension or protocol found.");
        return HandshakeState.NOT_MATCHED;
    }
    ...
    private HandshakeState containsRequestedProtocol(String str) {
        for (IProtocol iProtocol : this.knownProtocols) {
            if (iProtocol.acceptProvidedProtocol(str)) {
                ...
                LogUtil.logD("acceptHandshake - Matching protocol found: {}", iProtocol);
                return HandshakeState.MATCHED;
            }
        }
        return HandshakeState.NOT_MATCHED;
    ...
    public HandshakeState acceptHandshakeAsClient(ClientHandshake clientHandshake, ServerHandshake serverHandshake) throws InvalidHandshakeException {
        if (!basicAccept(serverHandshake)) {
            LogUtil.logD("acceptHandshakeAsClient - Missing/wrong upgrade or connection in handshake.");
            return HandshakeState.NOT_MATCHED;
        }
        if (!clientHandshake.hasFieldValue(SEC_WEB_SOCKET_KEY) || !serverHandshake.hasFieldValue(SEC_WEB_SOCKET_ACCEPT)) {
            LogUtil.logD("acceptHandshakeAsClient - Missing Sec-WebSocket-Key or Sec-WebSocket-Accept");
            return HandshakeState.NOT_MATCHED;
        }
        if (!generateFinalKey(clientHandshake.getFieldValue(SEC_WEB_SOCKET_KEY)).equals(serverHandshake.getFieldValue(SEC_WEB_SOCKET_ACCEPT))) {
            LogUtil.logD("acceptHandshakeAsClient - Wrong key for Sec-WebSocket-Key.");
            return HandshakeState.NOT_MATCHED;
        }
        HandshakeState handshakeState = HandshakeState.NOT_MATCHED;
        ...
        while (true) {
            ...
            if (next.acceptProvidedExtensionAsClient(fieldValue)) {
                this.extension = next;
                handshakeState = HandshakeState.MATCHED;
                LogUtil.logD("acceptHandshakeAsClient - Matching extension found: {}", this.extension);
                break;
            }
        }
        ...
        LogUtil.logD("acceptHandshakeAsClient - No matching extension or protocol found.");
        return HandshakeState.NOT_MATCHED;
    }
    ...
    public HandshakeBuilder postProcessHandshakeResponseAsServer(ClientHandshake clientHandshake, ServerHandshakeBuilder serverHandshakeBuilder) throws InvalidHandshakeException {
        ...
        serverHandshakeBuilder.put(g.I, "TooTallNate Java-WebSocket");
        serverHandshakeBuilder.put(g.A, getServerTime());
    ...
    private ByteBuffer createByteBufferFromFramedata(Framedata framedata) {
        ...
        byte bFromOpcode = (byte) (fromOpcode(framedata.getOpcode()) | ((byte) (framedata.isFin() ? a.g : 0)));
        if (framedata.isRSV1()) {
    ...
    private TranslatedPayloadMetaData translateSingleFramePayloadLength(ByteBuffer byteBuffer, Opcode opcode, int i, int i2, int i3) throws InvalidFrameException, IncompleteException, LimitExceededException {
        ...
        if (opcode == Opcode.PING || opcode == Opcode.PONG || opcode == Opcode.CLOSING) {
            LogUtil.logD("Invalid frame: more than 125 octets");
    ...
    private void translateSingleFrameCheckLengthLimit(long j) throws LimitExceededException {
        if (j > 2147483647L) {
            LogUtil.logD("Limit exedeed: Payloadsize is to big...");
        ...
        if (j <= this.maxFrameSize) {
            ...
            LogUtil.logD("Limit underflow: Payloadsize is to little...");
            throw new LimitExceededException("Payloadsize is to little...");
        }
        LogUtil.logD("Payload limit reached. Allowed: {}" + this.maxFrameSize + " Current: " + j);
    ...
    private void translateSingleFrameCheckPacketSize(int i, int i2) throws IncompleteException {
        ...
        LogUtil.logD("Incomplete frame: maxpacketsize < realpacketsize");
    ...
    public void processFrame(WebSocketImpl webSocketImpl, Framedata framedata) throws InvalidDataException {
        ...
        if (this.currentContinuousFrame != null) {
            LogUtil.logE("Protocol error: Continuous frame sequence not completed.");
        ...
        if (opcode == Opcode.TEXT) {
            ...
            LogUtil.logE("non control or continious frame expected");
    ...
    private void processFrameContinuousAndNonFin(WebSocketImpl webSocketImpl, Framedata framedata, Opcode opcode) throws InvalidDataException {
        if (opcode != Opcode.CONTINUOUS) {
            ...
            LogUtil.logE("Protocol error: Continuous frame sequence was not started.");
            throw new InvalidDataException(1002, "Continuous frame sequence was not started.");
        }
        if (opcode == Opcode.TEXT && !Charsetfunctions.isValidUTF8(framedata.getPayloadData())) {
            LogUtil.logE("Protocol error: Payload is not UTF8");
    ...
    private void logRuntimeException(WebSocketImpl webSocketImpl, RuntimeException runtimeException) {
        LogUtil.logE("Runtime exception during onWebsocketMessage", runtimeException);
    ...
    private void processFrameIsFin(WebSocketImpl webSocketImpl, Framedata framedata) throws InvalidDataException {
        if (this.currentContinuousFrame == null) {
            LogUtil.logD("Protocol error: Previous continuous frame sequence not completed.");
            throw new InvalidDataException(1002, "Continuous frame sequence was not started.");
        }
        addToBufferList(framedata.getPayloadData());
        checkBufferLimit();
    ...
    private void checkBufferLimit() throws LimitExceededException {
        ...
        clearBufferList();
        LogUtil.logD("Payload limit reached. Allowed: {} " + this.maxFrameSize + " Current: " + byteBufferListSize);
